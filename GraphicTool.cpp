...
void GraphicTool::blit(FrameBuffer* dst, const Rect& dstRect, const Texture& src, const Rect& srcRect)
{
    ...
    auto srcPixels = reinterpret_cast<const uint8_t*>(src.getPixels());
    ...
    for (int row = 0; row < height; ++row)
    {
        ...
        auto srcStart = srcPixels + srcX + srcY * src.getPitch();
        auto dstStart = dstPixels + dstX + dstY * dst->getPitch();
